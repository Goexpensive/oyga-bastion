<?php
// [ux_banner]
function wpo_uxbannerShortcode( $atts, $content = null ){
  global $flatsome_opt;
  $bannerid = rand();
  $relative_path = get_stylesheet_directory_uri();
  ...
  ob_start();
   ...
   if(!$alt) $alt = "";
   $background = "";
    if (strpos($bg,'/img/') !== false ) {
      $background = $relative_path . $bg;
    }
      ...
      <?php if($bg || $bg_color){ ?>
       ...
       <video id="banner_video" class="ux-banner-video hide-for-small" <?php if($parallax) echo $parallax; ?> poster="<?php echo $background; ?>" preload="auto" autoplay <?php if($video_sound == 'false') echo "muted='muted'"; ?>
        <?php if($video_loop == 'true') echo 'loop'; ?>>
            <source src="<?php echo $relative_path . $video_mp4; ?>" type="video/mp4">
            <source src="<?php echo $relative_path . $video_webm; ?>" type="video/webm">
            <source src="<?php echo $relative_path . $video_ogg; ?>" type="video/ogg">
        </video>
      <?php } ?>
...
function wpbastion_add_shortcodes() {
   
   add_shortcode('ux_banner', 'wpo_uxbannerShortcode');;

}
add_action( 'after_setup_theme', 'wpbastion_add_shortcodes', 200 );
